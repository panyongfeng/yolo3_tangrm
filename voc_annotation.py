...
def convert_to_tfrecord(xml, record_writer):
    ...
    print(name)
    image_data =  tf.gfile.FastGFile("../VOCdevkit/VOC2007/JPEGImages/"+name+".jpg", 'rb').read()
    #image_data = tf.io.read_file(
    #    tf.io.gfile.glob('../VOCdevkit/VOC2007/JPEGImages/%s.jpg' % (name))[0])
    example = tf.train.Example(features=tf.train.Features(
        feature={
            'image/encoded':
            tf.train.Feature(bytes_list=tf.train.BytesList(value=[image_data])),
            'image/object/bbox/name':
            tf.train.Feature(bytes_list=tf.train.BytesList(value=[name.encode()])),
...
for clazz in classes:
    ...
    file = "Annotations"
    print(file)
    if tf.io.gfile.isdir('../VOCdevkit/%s/%s' % (clazz, file)):
        xmls = tf.io.gfile.glob('../VOCdevkit/%s/%s/*.xml' % (clazz, file))
